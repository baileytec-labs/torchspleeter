...
def split_to_parts(inputaudiofile,outputdir,instruments=2,models=[],verbose=False):
...
def main():
    ...
    parser.add_argument('-v','--verbose',help='Enable verbose/debug output',action='store_true',default=False)
    ...
    if args['modeldir'] is not None:
        ...
        args['modeldir']=[]
    outputfiles=split_to_parts(args['inputfile'],args['output'],args['number'],args['modeldir'],verbose=args['verbose'])
    if args['verbose']:
